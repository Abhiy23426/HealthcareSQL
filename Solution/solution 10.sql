...
Delimiter //
create procedure State_Recommendation(in state_name varchar(10))
	begin
		set @avg_insurance_patient_ratio = (select count(distinct patientID) from patient)/(select count(distinct companyID) from insurancecompany);
		with patient_count as 
		(select state, count(distinct patientID) as num_patients
		from (select * from address where state = state_name) as address
		join person using(addressID)
		join patient on person.personID = patient.patientID),

		company_count as 
		(select state, count(distinct companyID) as num_insurance_companies
		from (select * from address where state = state_name) as address
		join insurancecompany using(addressID))

		select *, (num_patients/num_insurance_companies) as 'insurance_patient_ratio', @avg_insurance_patient_ratio as avg_insurance_patient_ratio,
					if (num_patients/num_insurance_companies < @avg_insurance_patient_ratio, 'Recommended', 'Not Recommended') as 'Recommendation'
		from patient_count join company_count using(state);
	end //
delimiter ;

call State_Recommendation('OK');
...
*/
create table if not exists placeadded
...
Delimiter //
CREATE TRIGGER New_city_State
Before Insert on Address for each row
	Begin
		if new.addressID not in (select addressID from address) then
			if new.state not in (select distinct state from address)
				then insert into placeadded(PlaceName, PlaceType, TimeAdded) values ( new.state, 'State', now());
			end if;
			if new.city not in (select distinct city from address)
				then insert into placeadded(PlaceName, PlaceType, TimeAdded) values ( new.city, 'City', now());
			end if;
        end if;
	end //
delimiter ;
...
delimiter //
create trigger Medicine_Update_log
After Update on keep for each row
	Begin 
		if new.quantity != old.quantity
			then insert into keep_log values(default, old.medicineID, new.quantity - old.quantity);
		end if;
	end //
delimiter ;

select * from keep;
update keep set quantity = 5949 where pharmacyID = 1008 and medicineID = 1111;
