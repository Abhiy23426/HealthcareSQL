...
Write a stored procedure that returns “claimed higher than average” 
...
If the number of claims for the passed disease is 
higher than the average return “claimed higher than average” 
otherwise “claimed lower than average”.
*/

DELIMITER //
Create procedure Claim_type(IN disease_ID_IN int)
	BEGIN
		if (select count(distinct claimID) from treatment where diseaseID = disease_ID_IN) > (select count(distinct claimID)/count(distinct diseaseID) from treatment) 
			then select disease_ID_IN,'Claim Higher than average';
		else select disease_ID_IN, 'Claim Lower than average';
		end if;
	END //
DELIMITER ; 

call claim_type(15);
select count(distinct claimID) from treatment where diseaseID = 15;
select count(distinct claimID)/count(distinct diseaseID) from treatment;



/*Problem Statement 2:  
Joseph from Healthcare department has requested for an application 
which helps him get genderwise report for any disease. 
...
Where, more_treated_gender is either ‘male’ or ‘female’ 
based on which gender underwent more often for the disease, 
if the number is same for both the genders, the value should be ‘same’.
*/

DELIMITER //
Create procedure Disease_Detail(IN disease_ID_IN int)
	BEGIN
		select DiseaseName, Male_count, Female_Count, if( male_count > Female_count, 'Male', if (female_count > male_count, 'Female', 'Same')) as 'More_Treated_Gender' from 
		(select  count(gender) as Male_count from treatment join person on person.personID = treatment.patientID where diseaseID = disease_ID_IN and gender = 'male') as m
        join (select  count(gender) as Female_count from treatment join person on person.personID = treatment.patientID where diseaseID = disease_ID_IN and gender = 'Female') as f
        join (select diseaseName from disease where diseaseID = Disease_ID_IN) as d ;
	END //
DELIMITER ;
drop procedure Disease_Detail;
call Disease_Detail(10);
